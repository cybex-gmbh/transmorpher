...
class S3Helper implements CloudStorageInterface
{
    ...
    public function getOpenConfiguration(string $key): array
    {
        return [
            'cloud'   => $this->getS3(),
            'options' => [
                'Bucket' => config(sprintf('filesystems.disks.%s.bucket', config('transmorpher.disks.originals'))),
                'Key'    => $key,
            ],
        ];
    }
    ...
    public function getSaveConfiguration(string $destinationPath, string $fileName): array
    {
        return [
            ...
            'options' => [
                'dest'     => sprintf('s3://%s/%s',
                    config(sprintf('filesystems.disks.%s.bucket', config('transmorpher.disks.videoDerivatives'))),
                    $destinationPath
                ),
                'filename' => $fileName,
            ],
